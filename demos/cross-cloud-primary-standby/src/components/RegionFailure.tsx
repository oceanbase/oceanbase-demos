...
export default function RegionFailure({ resetTrigger }: RegionFailureProps) {
  ...
  // 获取翻译后的按钮文本（用于点击检测）
  const switchToPrimaryText = intl.formatMessage({
    id: "button.switchToPrimary",
  });
  const createBackupText = intl.formatMessage({
    id: "button.createCrossCloudBackup",
  });
  const releaseInstanceText = intl.formatMessage({
    id: "button.releaseInstance",
  });
  ...
  const handleStateChange = (event: React.MouseEvent) => {
    ...
    if (
      state === "initial" &&
      (text.includes("切换为主实例") || text.includes(switchToPrimaryText))
    ) {
    ...
    if (
      state === "recovered" &&
      (text.includes("创建跨云主备库") || text.includes(createBackupText))
    ) {
    ...
    if (
      state === "recovered" &&
      (text.includes("释放实例") || text.includes(releaseInstanceText))
    ) {
    ...
    if (
      ...
      console.log(
        "地域故障 - Transitioning from state6 to afterReleasing state"
      );
      setState("afterReleasing");
      return;
    }
