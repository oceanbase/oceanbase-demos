...
export function MetricsPanel({ metrics, logs = [], theme }: MetricsPanelProps) {
    ...
    return (
      ...
      >
        ...
        >
          ...
          />
          <FormattedMessage id="metrics.title" defaultMessage="实时性能监控" />
        </h2>
        <div
  ...
  const primarySwitchMarkers: PrimarySwitchMarker[] = logs
    ...
    .map((log) => ({
      ...
      label: intl.formatMessage({
        id: "metrics.switchPrimary",
        defaultMessage: "切换主可用区",
      }),
    }));

  // 找到扩缩容阶段的区域（使用全部数据，基于详细的 scalingState）
  const phaseAreas: PhaseArea[] = [];
  let currentState: ScalingState | undefined =
    chartData[0]?.scalingState || "idle";
  ...
  chartData.forEach((point, idx) => {
    ...
    if (point.scalingState !== currentState || idx === chartData.length - 1) {
      // 计算结束时间
      const endTime =
        idx === chartData.length - 1 && point.scalingState === currentState
          ? point.time
          : point.time;
      ...
      if (currentState && currentState !== "idle") {
        ...
        switch (currentState) {
          case "scaling-out":
            label = intl.formatMessage({
              id: "metrics.phase.scaleOutAddSync",
              defaultMessage: "扩容：添加新Zone+同步",
            });
            color = "#eab308"; // 黄色
            break;
          case "switching-primary":
            label = intl.formatMessage({
              id: "metrics.phase.switchPrimary",
              defaultMessage: "切换主可用区",
            });
            color = "#a855f7"; // 紫色
            break;
          case "scaling-out-migrating":
            label = intl.formatMessage({
              id: "metrics.phase.scaleOutDelete",
              defaultMessage: "扩容：删除旧Zone",
            });
            color = "#f97316"; // 橙色
            break;
          case "scaling-in":
            label = intl.formatMessage({
              id: "metrics.phase.scaleInAddSync",
              defaultMessage: "缩容：添加新Zone+同步",
            });
            color = "#eab308"; // 黄色
            break;
          case "scaling-in-migrating":
            label = intl.formatMessage({
              id: "metrics.phase.scaleInDelete",
              defaultMessage: "缩容：删除旧Zone",
            });
            color = "#f97316"; // 橙色
            break;
        }
  ...
  if (chartData.length > 0) {
    ...
    chartData.forEach((point, idx) => {
      ...
      if (scenarioChanged || isLastPoint) {
        // 确定结束时间
        const endTime =
          scenarioChanged && idx > 0 ? chartData[idx - 1].time : point.time;
        ...
        if (endTime >= scenStart) {
          ...
          if (label) {
            scenarioAreas.push({
              ...
              phase: label,
              color,
              isCurrentPhase: isLastPoint && !scenarioChanged,
            });
          }
        }
  ...
  const visibleScenarioAreas = scenarioAreas
    ...
    .map((area) => ({
      ...
      start: Math.max(area.start, chartStartTime),
      // 如果是当前阶段，延伸到最后
      end: area.isCurrentPhase
        ? Math.max(area.end, chartEndTime)
        : Math.min(area.end, chartEndTime),
    }));
  ...
  let lastScalingType: "scale-out" | "scale-in" | null = null;
    ...
    if (prevState === "idle" && currentState === "scaling-out") {
      ...
      if (!addedEvents.has(eventKey)) {
        scalingEventMarkers.push({
          time: point.time,
          label: intl.formatMessage({
            id: "metrics.event.startScaleOut",
            defaultMessage: "开始扩容",
          }),
          type: "scale-out",
          color: "#22c55e", // 绿色
        });
        lastScalingType = "scale-out";
        addedEvents.add(eventKey);
      }
    }
    // 检测扩容后的切主（从 scaling-out 到 switching-primary）
    else if (
      prevState === "scaling-out" &&
      currentState === "switching-primary"
    ) {
      if (lastScalingType === "scale-out") {
        ...
        if (!addedEvents.has(eventKey)) {
          scalingEventMarkers.push({
            time: point.time,
            label: intl.formatMessage({
              id: "metrics.event.scaleOutSwitchPrimary",
              defaultMessage: "扩容后切主",
            }),
            type: "switch-primary",
            color: "#a855f7", // 紫色
          });
    ...
    // 检测从 idle 到 scaling-in（开始缩容）
      ...
      if (!addedEvents.has(eventKey)) {
        scalingEventMarkers.push({
          time: point.time,
          label: intl.formatMessage({
            id: "metrics.event.startScaleIn",
            defaultMessage: "开始缩容",
          }),
          type: "scale-in",
          color: "#f59e0b", // 橙色
        });
        lastScalingType = "scale-in";
        addedEvents.add(eventKey);
      }
    }
    // 检测缩容后的切主（从 scaling-in 到 switching-primary）
    else if (
      prevState === "scaling-in" &&
      currentState === "switching-primary"
    ) {
      if (lastScalingType === "scale-in") {
        ...
        if (!addedEvents.has(eventKey)) {
          scalingEventMarkers.push({
            time: point.time,
            label: intl.formatMessage({
              id: "metrics.event.scaleInSwitchPrimary",
              defaultMessage: "缩容后切主",
            }),
            type: "switch-primary",
            color: "#a855f7", // 紫色
          });
  ...
  return (
    <div
      className={`rounded-lg p-4 h-full flex flex-col border ${
        theme === "dark"
          ? "bg-slate-900 border-slate-800"
          : "bg-white border-gray-200"
      }`}
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div
          className={`rounded-lg p-2 flex-1 ${
            theme === "dark" ? "bg-slate-800/30" : "bg-gray-100"
          }`}
        >
                  ...
                  ? scenarioAreasWithPosition.map((area, idx) => {
                      ...
                      if (idx > 0) {
                        ...
                        const prevEndPercent =
                          prevArea.startPercent + prevArea.widthPercent;
                        const gap = finalStartPercent - prevEndPercent;

                        // 如果有明显缺口，从前一个区域结束位置开始
                        if (gap > 0.5) {
                          const originalEndPercent =
                            finalStartPercent + finalWidthPercent;
                          finalStartPercent = prevEndPercent;
                          finalWidthPercent =
                            originalEndPercent - finalStartPercent;
                        }
                      }

                      // 边界检查
                      finalStartPercent = Math.max(
                        0,
                        Math.min(100, finalStartPercent)
                      );
                      finalWidthPercent = Math.max(
                        0,
                        Math.min(100 - finalStartPercent, finalWidthPercent)
                      );
                      ...
                      const actualWidth =
                        (finalWidthPercent / 100) * containerWidth;
                      ...
                      return (
                        ...
                        >
                          {showText && (
                            <span className="px-2">{area.phase}</span>
                          )}
                        </div>
                      );
                    })
